...
public class SteveProdStarter {
    ...
    void started(EndpointInfo endpointInfo, String managerUrl) {
        stopPrintingDots();
        String msg = " Done!" + sep() + HINT;
        println(msg);
        printURLs(endpointInfo, managerUrl);
    }

    void startedWithErrors(EndpointInfo endpointInfo, String managerUrl) {
        stopPrintingDots();
    ...
    private void printURLs(EndpointInfo info, String managerUrl) {
        println("Web interface (manager):");
        println("- " + managerUrl);
        // println("OCPP 1.6 SOAP endpoint:");
        // println("- " + info.getOcppSoap());
        println("OCPP-J WebSocket endpoint (replace chargeBoxId):");
        println("- " + info.getOcppWebSocket());
    }
